...
<script type="text/javascript" src="js/tiny-mce-popup.js"></script>
...
jQuery(document).ready(function () {			
	...
	if(jQuery('#altos-st-cid-zid').length > 0){
		...
		jQuery.getJSON(regions_codes_url,
			function(data){
				...
				if(jQuery('#D').length > 0){
					...
					jQuery.getJSON(available_dates_url,
					function(data){
						jQuery('#D').empty();
						jQuery('#D').append('<option value="dynamic">Dynamic - Update values every week</option>');					
						jQuery.each(data, function(key,value){
							jQuery('#D').append(jQuery('<option></option>').attr("value",key).text(value));
						});
					});	
				}
				else{
					...
					function(data){
						...
						if(jQuery('#endDate').attr ('cookie')){
							jQuery('#endDate').val(jQuery('#endDate').attr ('cookie'));
						}
					});						
				}
